...
function setupCatalog(){const g=$("#catalogGrid");if(!g)return;let cat="all",size="all",sort="popular";const sizeFilter=$("#sizeFilter");const sizeOptions={Подушки:[["50×70","50/70"],["70×70","70/70"],["40×60","40/60"]],Ковдри:[["Полуторні","Полуторні"],["Двоспальні","Двоспальні"],["Євро розмір","Євро розмір"]]};function updateSizeOptions(){if(!sizeFilter)return;const opts=sizeOptions[cat]||[];sizeFilter.innerHTML='<button class="chip active" type="button" data-size="all">Усі</button>'+opts.map(o=>`<button class="chip" type="button" data-size="${o[0]}">${o[1]}</button>`).join("");size="all";$$(".sizeChips .chip").forEach(b=>b.onclick=()=>{$$(".sizeChips .chip").forEach(x=>x.classList.remove("active"));b.classList.add("active");size=b.dataset.size;draw()})}function draw(){let a=[...P];if(cat!=="all")a=a.filter(x=>x.cat===cat);if(size!=="all"){if(cat==="Ковдри")a=a.filter(x=>x.group===size);else a=a.filter(x=>x.sizes.includes(size))}if(sort==="cheap")a.sort((x,y)=>x.price-y.price);if(sort==="expensive")a.sort((x,y)=>y.price-x.price);g.innerHTML=a.map(card).join("");bindImageFallback(g);$("#resultCount").textContent=`${a.length} товарів`;observe()}$$(".catFilter").forEach(b=>b.onclick=()=>{$$(".catFilter").forEach(x=>x.classList.remove("active"));b.classList.add("active");cat=b.dataset.cat;updateSizeOptions();draw()});$("#sortFilter")?.addEventListener("change",e=>{sort=e.target.value;draw()});const q=new URLSearchParams(location.search).get("cat");if(q){const b=$(`.catFilter[data-cat="${q}"]`);if(b){b.classList.add("active");cat=q}}updateSizeOptions();draw()}
