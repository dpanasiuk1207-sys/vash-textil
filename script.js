...
function setupProduct(){
 ...
 function show(i){if(!items.length)return;idx=(i+items.length)%items.length;const x=items[idx];if(x.type==="video"){img.hidden=true;vid.hidden=false;vid.poster=x.poster||"";vid.src=x.src;vid.muted=true;vid.autoplay=true;vid.loop=true;vid.playsInline=true;vid.load();vid.play().catch(()=>{})}else{vid.pause();vid.removeAttribute("src");vid.load();vid.hidden=true;img.hidden=false;img.src=x.src;img.alt=x.alt}if(counter)counter.textContent=`${idx+1} / ${items.length}`;$$('.media-thumb',gallery).forEach((b,n)=>b.classList.toggle('active',n===idx))}
